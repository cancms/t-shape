...
function boltSpecialPoints(boltDiameter, boltLength, boltQuFuEpsilon, boltFengZhiEpsilon, boltDuanLieEpsilon, boltQuFuForce, boltFengZhiForce, boltDuanLieForce, m, n) {
    ...
    // 螺栓屈服
    const Ty = boltLength * boltQuFuEpsilon; // 螺栓本身位移
    const deltaBy = luoshuanWeiyi(By, Ty, m, n);
    ; // 螺栓本身位移 + 翼缘位移
...
function calculateTShapeService(inputParams = {}) {
    ...
    // 计算中间值
    const intermediateData = prepareData();
    ...
    // 屈服曲率
    const Xy = intermediateData.flangeKeyCurvatures.Xy;
    const Xh = intermediateData.flangeKeyCurvatures.Xh;
    const Xm = intermediateData.flangeKeyCurvatures.Xm;
    const Xf = intermediateData.flangeKeyCurvatures.Xf;

    const My = intermediateData.flangeKeyMas.My;
    const Mh = intermediateData.flangeKeyMas.Mh;
    const Mm = intermediateData.flangeKeyMas.Mm;
    const Mu = intermediateData.flangeKeyMas.Mu;
    const Mp = intermediateData.flangeKeyMas.Mp;
...
// 根据曲率计算弯矩
    ...
    function calculateCas() {
        ...
        function calculateBoltForceAndMove(J52, J59) {
            ...
            // 从螺栓屈服荷载算出力F
            const FBy = intermediateData.boltKeyPointsData.By * J59 * J52;

            // 从螺栓峰值荷载算出力F
            const FBu = intermediateData.boltKeyPointsData.Bu * J59 * J52;

            // 从螺栓断裂荷载算出力F
            const FBf = intermediateData.boltKeyPointsData.Bf * J59 * J52;

            const Dy = intermediateData.boltKeyPointsData.Dy
            const Du = intermediateData.boltKeyPointsData.Du
            const Df = intermediateData.boltKeyPointsData.Df
    ...
    // R(F): 为受力（R列的值）， 参考插值表的值，用trend（excel函数）插值算法根据Ma（弯矩的值）得到 T（Δ2）的值
    // Δ2， T 列， 螺栓产生的位移
    function calculateDelta2(Rf) {
        const luoshanForceToDelta2 = intermediateData.CaValues.luoshanForceToDelta2;
        // 前面的 + 号把 toFixed 后的字符串转换回来成数字
        const FBy = +luoshanForceToDelta2.force.FBy.toFixed(0);
        const FBu = +luoshanForceToDelta2.force.FBu.toFixed(0);
        const Dy = +luoshanForceToDelta2.move.Dy.toFixed(2);
        const Du = +luoshanForceToDelta2.move.Du.toFixed(2);
    ...
    // 依赖 delta2， Δ2 （T列）
    // R(f): 为受力（R列的值）
    function calculateDelta1(Rf, theta, delta2) {
    ...
    function calculateForceAndDelta() {
        ...
        function flangeForceAndMove(moment,  theta) {
            ...
            // 螺栓位移 delta2 (T)
            const d2 = calculateDelta2(force);

            // 翼缘位移 delta1 (S)
            const d1 = calculateDelta1(force, theta, d2);
        ...
        flangePoints = flangePoints.map(i => {
            const forceMove = flangeForceAndMove(i.Ma, flangeService.calculateTheta(i.c));
            i.x = forceMove.move;
            i.y = forceMove.force;
